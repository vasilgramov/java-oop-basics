...
public class StudentsRepository {
    ...
    private void readData(String fileName) throws IOException {
        ...
        for (String line : lines) {
            ...
            if (!line.isEmpty() && matcher.find()) {
                ...
                try {
                    String[] splitScore = scoreStr.split("\\s+");
                    int[] scores = new int[splitScore.length];
                    for (int i = 0; i < splitScore.length; i++) {
                        scores[i] = Integer.parseInt(splitScore[i]);
                    }
    ...
    public void getStudentMarksInCourse(String courseName, String studentName) {
        ...
        double mark = this.courses.get(courseName).getStudentByName(studentName).getMarkByCourse(courseName);
    ...
    public void getStudentsByCourse(String courseName) {
        ...
        for (Map.Entry<String, Student> student : this.courses.get(courseName).getStudentByName().entrySet()) {
    ...
    private boolean isQueryForStudentPossible(String courseName, String studentName) {
        ...
        if (!this.courses.get(courseName).containsStudent(studentName)) {
    ...
    public void filterAndTake(String courseName, String filter, int studentsToTake) {
        ...
        Map<String, Student> studentByName = this.courses.get(courseName).getStudentByName();
        for (Map.Entry<String, Student> stringStudentEntry : studentByName.entrySet()) {
            marks.put(stringStudentEntry.getKey(), stringStudentEntry.getValue().getMarkByCourse(courseName));
        }
